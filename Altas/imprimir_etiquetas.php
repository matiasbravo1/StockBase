...
$sql = "SELECT * FROM altas WHERE `id_remito` = '$id_remito' AND `etiquetas` <> '0' ORDER BY id_alta DESC";
...
while($row = $result->fetch_assoc()) {
    ...
    $sql2 = "SELECT * FROM remitos WHERE `id_remito` = '$id_remito'";
    $result2 = $conn->query($sql2);
    $row2 = $result2->fetch_assoc();

    for ($i = 0; $i < $row["etiquetas"]; $i++){
        //echo ;
